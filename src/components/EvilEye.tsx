...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    const fromIntensity   = introFrom.intensity    ?? intensity;
    const fromPupilSize   = introFrom.pupilSize     ?? pupilSize;
    const fromIrisWidth   = introFrom.irisWidth     ?? irisWidth;
    const fromGlowIntensity = introFrom.glowIntensity ?? glowIntensity;
    const fromScale       = introFrom.scale         ?? scale;
    const fromNoiseScale  = introFrom.noiseScale    ?? noiseScale;
    const fromPupilFollow = introFrom.pupilFollow   ?? pupilFollow;
    const fromFlameSpeed  = introFrom.flameSpeed    ?? flameSpeed;
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uPupilSize: { value: fromPupilSize },
        uIrisWidth: { value: fromIrisWidth },
        uGlowIntensity: { value: fromGlowIntensity },
        uIntensity: { value: fromIntensity },
        uScale: { value: fromScale },
        uNoiseScale: { value: fromNoiseScale },
        uMouse: { value: [0, 0] },
        uPupilFollow: { value: fromPupilFollow },
        uFlameSpeed: { value: fromFlameSpeed },
    ...
    function update(time: number) {
      ...
      if (introDuration > 0) {
        ...
        const progress = easeInOut(Math.min(elapsed / introDuration, 1));
        program.uniforms.uIntensity.value    = lerp(fromIntensity,    intensity,    progress);
        program.uniforms.uPupilSize.value    = lerp(fromPupilSize,    pupilSize,    progress);
        program.uniforms.uIrisWidth.value    = lerp(fromIrisWidth,    irisWidth,    progress);
        program.uniforms.uGlowIntensity.value = lerp(fromGlowIntensity, glowIntensity, progress);
        program.uniforms.uScale.value        = lerp(fromScale,        scale,        progress);
        program.uniforms.uNoiseScale.value   = lerp(fromNoiseScale,   noiseScale,   progress);
        program.uniforms.uPupilFollow.value  = lerp(fromPupilFollow,  pupilFollow,  progress);
        program.uniforms.uFlameSpeed.value   = lerp(fromFlameSpeed,   flameSpeed,   progress);
      }
  ...
  }, [eyeColor, intensity, pupilSize, irisWidth, glowIntensity, scale, noiseScale, pupilFollow, flameSpeed, backgroundColor, introDuration, introFrom]);
